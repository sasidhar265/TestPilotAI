...
def convert_results(path: Path, manifest: dict, run_key: str) -> dict:
    ...
    for case_id, statuses in grouped.items():
        status = 'failed' if 'failed' in statuses else 'not-run' if 'not-run' in statuses else 'passed'
        results.append({'case_id': case_id, 'status': status, 'actual': f'{len(statuses)} mapped TRX test(s): ' + ', '.join(statuses)})
    return {'cycle_id': manifest['cycle_id'], 'run_key': run_key, 'build': manifest['build'], 'environment': manifest['environment'], 'results': results}
...
def main() -> int:
    ...
    if playwright:
        subprocess.run(['pwsh', str(playwright[0]), 'install', '--with-deps', 'chromium'], check=True, timeout=300)
    result = subprocess.run(['dotnet', 'test', str(project), '--no-build', '--no-restore', '--logger', 'trx;LogFileName=results.trx', '--results-directory', str(results)], cwd=output, timeout=600)
    converted = convert_results(results / 'results.trx', manifest, f"ci-{os.getenv('GITHUB_RUN_ID', 'local')}-{os.getenv('GITHUB_RUN_ATTEMPT', '1')}")
