...
def base_slide(prs, section, number):
    ...
    add_text(slide, section.upper(), 9.2, 0.22, 3.4, 0.3, 8, MUTED, False, align=PP_ALIGN.RIGHT)
    add_text(slide, f"{number:02d}", 12.2, 7.08, 0.55, 0.22, 8, MUTED, align=PP_ALIGN.RIGHT)
    return slide
...
def build():
    ...
    add_text(
        slide, "From requirements to\nautomation-ready tests", 0.72, 1.28, 8.6, 1.7, 34, TEXT, True
    )
    add_text(
        slide,
        "A governed GitHub Copilot test-design proof of concept",
        0.76,
        3.12,
        7.8,
        0.5,
    ...
    add_text(
        slide,
        "Manual test steps  ·  SpecFlow BDD  ·  Automation/manual feasibility",
        0.76,
        3.72,
        8.8,
        0.35,
    ...
    add_text(slide, "GITHUB COPILOT", 9.85, 2.75, 2.1, 0.3, 10, MINT, True, align=PP_ALIGN.CENTER)
    add_text(slide, "Sole AI runtime", 9.85, 3.2, 2.1, 0.3, 13, TEXT, True, align=PP_ALIGN.CENTER)
    add_text(
        slide,
    ...
    title(
        slide,
        "Accelerate the first test-design pass",
        "A focused assistant for quality engineers—not autonomous QA",
    )
    card(
        slide,
        "The challenge",
        "Requirements must be translated into consistent scenarios before automation starts. This is repetitive, variable, and review intensive.",
        0.6,
        2.0,
        3.8,
        3.6,
    ...
    card(
        slide,
        "The intervention",
        "Generate 4–5 risk-focused scenarios with steps, expected results, SpecFlow Gherkin, synthetic data, and feasibility decisions.",
        4.75,
        2.0,
        3.8,
        3.6,
    ...
    card(
        slide,
        "The outcome",
        "Faster initial design, clearer automation candidates, explicit manual coverage, and controlled export or Jira publication.",
        8.9,
        2.0,
        3.8,
        3.6,
    ...
    card(
        slide,
        "Designed for review",
        "Generated output is a draft. The tester owns coverage, domain accuracy, automation implementation, and execution evidence.",
        8.45,
        2.05,
        3.9,
        1.75,
    ...
    card(
        slide,
        "Predictable POC",
        "4–5 high-level scenarios keep latency and Copilot premium-request use practical.",
        8.45,
        4.05,
        3.9,
        1.55,
    ...
    title(
        slide, "Live execution pipeline", "Local orchestration with one approved hosted AI runtime"
    )
    nodes = [
    ...
    for i, (num, name, detail) in enumerate(nodes):
        ...
        if i < len(nodes) - 1:
            add_text(slide, "→", x + 1.77, 2.93, 0.34, 0.35, 18, CYAN, True, align=PP_ALIGN.CENTER)
    add_text(slide, "LOCAL EXECUTION", 0.58, 4.42, 5.75, 0.25, 9, CYAN, True, align=PP_ALIGN.CENTER)
    add_text(slide, "GITHUB-HOSTED", 8.92, 4.42, 1.72, 0.25, 9, VIOLET, True, align=PP_ALIGN.CENTER)
    add_text(
        slide, "LOCAL VALIDATION", 10.98, 4.42, 1.72, 0.25, 9, MINT, True, align=PP_ALIGN.CENTER
    )
    add_text(
        slide,
        "Only the restricted generation request leaves the local application. Jira is a separate, explicit workflow.",
        1.0,
        5.25,
        11.3,
        0.5,
    ...
    title(
        slide, "Three distinct layers", "Clear accountability for model, behavior, and control flow"
    )
    card(
        slide,
        "LLM",
        "Selection: organization-default\nGateway: GitHub Copilot\nEntitlement: signed-in organization user\nNo direct provider API key",
        0.6,
        2.0,
        3.8,
        3.75,
    ...
    title(
        slide,
        "Copilot-only and fail closed",
        "Controls are implemented in code, not only documented",
    )
    bullet_list(
        slide,
        [
    ...
    add_text(slide, "approved runtime", 9.82, 3.92, 2.1, 0.3, 11, TEXT, align=PP_ALIGN.CENTER)
    ...
    card(
        slide,
        "Input",
        "A registered customer resets a password using a 15-minute email link. Used links cannot be reused; password policy applies; successful reset invalidates sessions.",
        0.6,
        1.9,
        4.15,
        3.9,
    ...
    card(
        slide,
        "Presenter focus",
        "Show the small risk-based suite, point out classification chips, inspect rationale, then export JSON or CSV.",
        8.5,
        1.9,
        4.15,
        3.9,
    ...
    title(
        ...
        "The agent explains—not merely labels—the execution decision",
    )
    card(
        slide,
        "Automation",
        "Repeatable and deterministic\nStable UI/API/system interface\nObservable assertions\nHigh rerun value\nExamples: authentication rules, link states, session invalidation",
        0.75,
        1.95,
        5.7,
        3.95,
    ...
    add_text(
        slide,
        "Guardrail: complexity alone is not a valid reason to classify a test as manual.",
        1.0,
        6.25,
        11.3,
        0.35,
    ...
    card(
        slide,
        "Copy",
        "Copy an individual Gherkin scenario or a complete SpecFlow Feature block to the clipboard.",
        0.6,
        2.0,
        2.8,
        3.4,
    ...
    card(
        slide,
        "Export",
        "Download the validated suite as CSV or JSON for review, evidence, or downstream transformation.",
        3.7,
        2.0,
        2.8,
        3.4,
    ...
    card(
        slide,
        "Select",
        "Choose which scenarios are eligible for publication rather than sending the complete draft.",
        6.8,
        2.0,
        2.8,
        3.4,
    ...
    card(
        slide,
        "Jira",
        "Attach selected cases and add a summary only after the tester explicitly initiates publishing.",
        9.9,
        2.0,
        2.8,
        3.4,
    ...
    title(
        ...
        "Transparent limitations support responsible adoption",
    )
    bullet_list(
        slide,
        [
    ...
    title(
        ...
        "Keep the narrative focused on value, controls, and measurable next steps",
    )
    steps = [
        ("00–01", "Problem and value"),
        ("01–02", "Architecture and governance"),
        ("02–04", "Normal scenario generation"),
        ("04–07", "SpecFlow BDD + copy"),
        ("07–08", "Automation/manual mix"),
        ("08–09", "Export and Jira"),
    ...
    for i, (time, label) in enumerate(steps):
        ...
        line = slide.shapes.add_shape(
            MSO_SHAPE.RECTANGLE, Inches(7.7), Inches(y + 0.12), Inches(4.2), Inches(0.02)
        )
        line.fill.solid()
    ...
    title(
        slide,
        "Move from POC to a measured pilot",
        "Validate usefulness, edit effort, latency, governance, and adoption",
    )
    cards = [
        ...
        (
            "4",
            "Review controls",
            "Agree Copilot policy, model, budget, privacy, logs, and deployment.",
        ),
    ...
    add_text(
        slide,
        "A governed path from requirements to an initial automation-ready test design using the organization’s GitHub Copilot capability.",
        0.8,
        3.22,
        8.5,
        0.8,
