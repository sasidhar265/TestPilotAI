...
async function loadCompanyDocument(id) {
  ...
  try {
    const response = await fetch(`/api/documentation/company/${encodeURIComponent(id)}`, {
      credentials: "same-origin",
      signal: controller.signal,
    });
    if (response.redirected || response.status === 401 || response.status === 403) {
      throw new Error("Your session has expired. Sign in again to view the planning documents.");
    }
    if (response.status === 404 || response.status === 503) {
      throw new Error(
        "This planning document is unavailable in this deployment. Contact the administrator to restore the documentation files.",
      );
    }
    if (!response.ok)
      throw new Error("The planning document could not be loaded. Please try again.");
    const documentData = await response.json();
    if (!controller.signal.aborted) reader.innerHTML = renderMarkdown(documentData.content);
  } catch (error) {
    ...
    const message =
      error instanceof TypeError
        ...
        : error.message;
    reader.innerHTML = `<p class="doc-loading" role="alert">${esc(message)}</p>`;
  }
}
...
fetch("/api/agents")
  ...
  .catch(() => {
    document.getElementById("agent-list").innerHTML =
      '<article class="card"><h3>Agent list unavailable</h3><p>Reload this page when the application service is available. The guide above explains the usual generation flow.</p></article>';
    document.querySelector("#status .status-copy").textContent = "Agent list unavailable";
  });

const navigationLinks = [...document.querySelectorAll('.docs-nav a[href^="#"]')],
