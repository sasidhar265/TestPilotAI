...
function syncReviewControls() {
  ...
  document.querySelectorAll('.toggle-case-review, .save-case-review, .case-review-comments').forEach(control => control.disabled = disabled);
}
...
$('cases').addEventListener('submit', async event => {
  ...
  if (stepDefinitionTask) {
    status.textContent = 'Wait for C# pack generation to finish or cancel it before revising the suite.';
    return;
  }
  ...
  try {
    await api('/api/reviews', {request: source, suite: snapshot, comments, test_case_id: testCaseId}, options);
    saved = true;
    ...
    panel.closest('.case-review').querySelector('.toggle-case-review').setAttribute('aria-expanded', 'true');
