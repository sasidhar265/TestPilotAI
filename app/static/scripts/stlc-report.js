...
function calculateStlcReport(currentSuite, source, validation) {
  ...
  const mappings = (test) => [
    ...new Set(
      (test.acceptance_criteria_covered || []).map((value) => value.trim()).filter(Boolean),
    ),
  ];
  const rules = [
    ...new Map((source?.business_rules || []).map((rule) => [normalize(rule.id), rule])).values(),
  ];
  const matches = (value, id) =>
    normalize(value) === normalize(id) || normalize(value).startsWith(normalize(id) + ":");
  const rows = rules.map((rule) => ({
    ...rule,
    cases: cases.filter((test) => mappings(test).some((value) => matches(value, rule.id))),
  }));
  const linked = cases.filter((test) => mappings(test).length);
  const distribution = (key) =>
    cases.reduce((counts, test) => {
      counts[test[key]] = (counts[test[key]] || 0) + 1;
      return counts;
    }, {});
  return {
    total: cases.length,
    rows,
    covered: rows.filter((row) => row.cases.length).length,
    linked: linked.length,
    unlinked: cases.filter((test) => !mappings(test).length),
    additional: [...new Set(cases.flatMap(mappings))].filter(
      (value) => !rules.some((rule) => matches(value, rule.id)),
    ),
    priority: distribution("priority"),
    category: distribution("category"),
    mode: distribution("execution_mode"),
    groups: new Set(cases.map((test) => test.scenario_group || "General scenario")).size,
    highRiskUnlinked: cases.filter(
      (test) => ["P0", "P1"].includes(test.priority) && !mappings(test).length,
    ),
    bdd: cases.filter((test) => test.execution_mode === "automation" && test.gherkin?.trim())
      .length,
...
if (typeof window !== "undefined") {
  window.renderStlcReport = () => {
    ...
    const pct = (count, total) =>
      total ? `${Math.round((count * 1000) / total) / 10}%` : "Unavailable";
    const metric = (value, label, detail) =>
      `<article class="stlc-metric"><strong>${esc(String(value))}</strong><span>${esc(label)}</span><small>${esc(detail)}</small></article>`;
    const list = (values) =>
      values.length
        ? values.map((value) => `<li>${esc(value)}</li>`).join("")
        : "<li>None recorded.</li>";
    const bars = (title, values) =>
      `<section><h4>${esc(title)}</h4>${Object.entries(values)
        .map(
          ([label, count]) =>
            `<div class="stlc-bar"><span>${esc(label)}</span><meter min="0" max="${report.total}" value="${count}" aria-label="${esc(label)}"></meter><strong>${count} · ${pct(count, report.total)}</strong></div>`,
        )
        .join("")}</section>`;
    const ruleRows = report.rows
      .map(
        (row) =>
          `<tr><th scope="row">${esc(row.id)}</th><td>${esc(row.description)}</td><td>${row.cases.length ? row.cases.map((test) => esc(test.id)).join(", ") : "No linked tests"}</td><td>${row.cases.length ? "Covered" : "Gap"}</td></tr>`,
      )
      .join("");
    ...
    $("metrics-dashboard").innerHTML = `
      ...
      <section class="stlc-section"><h3>Requirement-to-test traceability</h3><p>Coverage uses unique business rules captured with this suite. A mapping records design coverage; it does not prove the requirement passed execution.</p>
        ...
        <details><summary>Other criterion mappings (${report.additional.length})</summary><p>These mappings are not matched to the source business-rule inventory and do not increase its coverage percentage.</p><ul>${list(
          report.additional.map(
            (value) =>
              `${value} · ${suite.test_cases
                .filter((test) =>
                  (test.acceptance_criteria_covered || []).some(
                    (mapping) => mapping.trim() === value,
                  ),
                )
                .map((test) => test.id)
                .join(", ")}`,
  ...
  $("show-rule-coverage").onclick = () => {
    window.renderStlcReport();
    $("metrics-dashboard").scrollIntoView({ behavior: "smooth", block: "start" });
  };
  window.renderStlcReport();
}
