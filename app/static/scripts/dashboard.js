...
(() => {
  ...
  function renderRun(item, open) {
    ...
    return `<details class="execution-run" data-run-id="${esc(item.id)}"${open.has(item.id) ? ' open' : ''}><summary>${badge(item.status)}<span class="bdd-run-name">Repository BDD<span>${esc(date(item.started_at))}</span></span><span class="bdd-run-counts">${summary}</span><span class="bdd-run-duration">${duration(item.duration_ms)}</span><span aria-hidden="true" class="bdd-chevron">⌄</span></summary><div class="bdd-run-expanded"><button class="secondary" type="button" data-chart-run="${esc(item.id)}">View results</button><dl class="run-details">${fields}</dl>${d.error ? `<p class="run-error">${esc(d.error)}</p>` : ''}${reportLink(d)}${d.output ? `<details><summary>Execution output</summary><pre class="run-output">${esc(d.output)}</pre></details>` : ''}</div></details>`;
  }
